# GA1.1-Onyx3

# subtract function:
subtract(a, b):
subtracts input b from input a. for example: subtract(8, 2) outputs 6
# distance-from-zero function: 
distance from zero function, distance_from_zero(x):
This function computes the distance from zero from the given variable x.
It breaks if there are more or less than 1 variable given.

# add function:
adds up 2 numbers.
input: two inputs from the same type --> add( a, b )
examples: add(4,5) -- 4+5 = 9
breaks: inputs have to be the same type.
# sum function
The function sum_list sums the elements in the given array. 
For example sum_list([10, 5, 5]) adds the items and returns 20 in this case.

# multiply function
The function "multiply" multiplies the two variables you define.
For example multiply(6,5) multiplies 6 and 5.
The function will break down if there are more or less than two variables defined.

# square function
The function square, uses the multiply function to multiply the given number with itself (square).
example: square(5) --> 5*5 = 25 - output = 25

